...
MAIL_PASSWORD = 'mynameisbluetooth'
)
mail = Mail(app)
         ...
         for users in hg:
             ...
             if name == "CodeForces":
                 if users.codeforces == 1 and i["in_24_hours"] == "Yes":
                     mail.send_message('Hurry!! Contest in 24 hours',sender = 'abhishekjoshi2691999@gmail.com',recipients = [users.email],body = 'Hi ' + users.username + ' your  ' + 'contest is in 24 hours on ' +  i["site"])
             if name == "CodeChef":
...
def register():
    if request.method == "POST":
        ...
        mail.send_message('Hi ' + username  + ' you are now registered on Smart Contest Reminder Portal' ,sender = 'abhishekjoshi2691999@gmail.com',recipients = [email],body = 'Welcome to our website Smart Contest Reminder , Here you can get details of your upcoming contest and get prediction for the no of questions to be done to reach a particular rating')
...
@app.route('/contestReminder')
def contestReminder():
    return render_template('contestReminder.html')
...
@app.route("/ratingPrediction",methods = ['GET','POST'])
def ratingPrediction():
    return render_template('ratingPrediction.html')
...
sched.add_job(se,'interval',minutes= 1)
sched.start()
